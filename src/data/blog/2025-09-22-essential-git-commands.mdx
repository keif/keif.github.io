...
modDatetime: 2025-12-18T16:55:49.529Z
...
Working with Git day-to-day can feel like second nature—until you hit one of those moments where you wish you knew the _right_ command.

After years of tripping over the same pain points, here's a practical list of Git commands that act like superpowers.

---

## 1. `git reflog` – _The Time Machine_
...
- `git switch -c new-branch` → friendlier than `git checkout -b`
    - Why? Because `git switch` is dedicated just to switching/creating branches, while `git checkout` is overloaded (it can also reset files, detach HEAD, etc.). This makes `switch` safer, clearer, and easier to learn. I'm still working on getting this to replace my alias usage of `git co -b`
...
These aren't obscure tricks — they're practical tools that save time, frustration, and sometimes entire projects (seriously, `git reflog`).
Mastering them means fewer “oh no” moments and faster context switching.

Up next on "need to learn/master" - `git worktree` - which sounds pretty interesting.
